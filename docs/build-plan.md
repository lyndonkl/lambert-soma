...
| PR | Title | What lands | Proof | Size |
|---|---|---|---|---|
| 04 | **Cell Protocol spec + cell-in-a-box** | `docs/protocols/cell.md`: BIRTH (briefing = first task-log entry) · TASK (claimed bead + briefing; later tasks as log notifications) · DIALOGUE (invitation event, turn events, RESOLVED emission) · DONE (Stop-conditions; explicit finish signal — never inferred from summaries, EXP-002) · DEATH (harness-side reflection trigger; `remember` for salient moments). `soma resume <run_id>` (SDK reconstruction: same persistence_dir + conversation_id). Conformance harness that drives one cell purely via synthetic logs | **human gate: spec review**; harness runs against PR-02 engine with golden transcripts; crash-resume check (kill mid-task, resume, continues not restarts) | M |
| 05 | Archetype loader + validation | SDK file-agent format adopted + soma validation (collisions, top-level `soma_*` keys, tier profile exists, tools); `soma archetypes list`; archetype sources = project `.agents/agents` + user library `~/.agents/agents` — **soma ships no roles** (G9); docs/archetypes.md | loader fixtures incl. invalid files | M |
...
| 07 | Skills wiring + audit gate | SDK skill loading wired at call time (project `.agents/skills` + user `~/.agents/skills`); archetypes mount skills by name; `soma skills list`/`audit` (render-time exec, always-on token bombs, shell-risk, size) — **no shipped skills** (G9) | audit fixture tests; **human gate: audit report in-PR** | M |
...
| 16b | Cell workflow guidance — default, overridable skill | a soma-shipped **default** beads-workflow skill the user can override (claim-before-work, close-when-done, note-vs-do, when to decompose), researched against the tier models' prompting guidance; decided beside PR-16 so team-side pouring and cell-side ordering (PR-11b) split responsibilities deliberately | adherence EXP on scripted tasks: decomposes when it should, not when it shouldn't | S |
...
| 18b | Mode overlay prompts — researched + data-grounded | task-mode and dialogue-mode overlays (`cells.py`) revised against published prompting guidance for the tier models in use, sources cited next to the text; overlays considered for promotion out of code into soma-shipped default files the user can override (archetype/skill shape) | **EXP**: same scripted dialogues, overlay A vs B — turn length, question quality, clean RESOLVED, task-overlay adherence; **turn cap grounded in the dialogue-length data from the same EXP** (today's 12 is unresearched); adopt from measurement | S |
